...
function fileIcon(ext: string): string {
  ...
  if (["jpg","jpeg","png","gif","webp","bmp","svg","ico","avif","heic"].includes(e)) return "🖼️";
  if (["mp3","wav","flac","aac","ogg","m4a","opus"].includes(e)) return "🎵";
  if (["mp4","avi","mkv","mov","wmv","flv","webm"].includes(e)) return "🎬";
  if (["zip","rar","7z","tar","gz","bz2","xz"].includes(e)) return "📦";
  if (["xlsx","xls","csv","ods"].includes(e)) return "📊";
  if (["docx","doc","pdf","md","txt","rtf"].includes(e)) return "📄";
  if (["js","ts","jsx","tsx","py","rs","go","java","cpp","c","cs","html","css"].includes(e)) return "💻";
  if (["exe","msi","bat","cmd","ps1","sh"].includes(e)) return "⚙️";
  if (["json","yaml","yml","toml","xml","ini","env"].includes(e)) return "⚙️";
  return "📁";
}
      ...
      <div className={`dz-target${isDragging ? " dz-target--active" : ""}${files.length > 0 ? " dz-target--has-files" : ""}`}>
        {files.length === 0 ? (
          <div className="dz-empty">
            <span className="dz-drop-icon">📥</span>
          ...
          <div className="dz-list">
            {files.map(f => (
              <div key={f.path} className="dz-row">
                <span className="dz-file-icon">{fileIcon(f.ext)}</span>
